...
class TestSumProduct(unittest.TestCase):
    ...
    def test_autograd(self):
        for example in self.examples:
            ...
            for j_precompute in [False, True]:
                with self.subTest(example=str(example), j_precompute=j_precompute):
                    fgg = example.fgg
                    ...
                    def f(*physicals):
                        opts = {'method': 'newton', 'tol': 1e-6, 'kmax': 100,
                                'semiring': RealSemiring(dtype=torch.double), 'j_precompute': j_precompute}
        ...
        for example in self.examples:
            ...
            for j_precompute in [False, True]:
                with self.subTest(example=str(example), j_precompute=j_precompute):
                    ...
                    def f(*physicals):
                        opts = {'method': 'newton', 'tol': 1e-6, 'kmax': 100,
                                'semiring': LogSemiring(dtype=torch.double), 'j_precompute': j_precompute}
    ...
    def test_infinite_gradient(self):
        ...
        for j_precompute in [False, True]:
            with self.subTest(j_precompute= j_precompute):
    ...
    def test_sum_product(self):
        for method in ['fixed-point', 'linear', 'newton']:
            for j_precompute in [False, True]:
                with self.subTest(method=method, j_precompute=j_precompute):
                    for example in self.examples:
