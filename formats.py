...
def fgg_to_json(g):
    ...
    for gr in g.all_rules():
        nodes = sorted(gr.rhs().nodes(), key=lambda node: node.label().name())
        node_nums = {v:i for (i,v) in enumerate(nodes)}
        jr = {
            'lhs': gr.lhs().name(),
            'rhs': {
...
def factorgraph_to_dot(g: FactorGraph, node_nums, edge_nums, factor_formats=None, lhs=None):
    ...
    for v in g.nodes():
        dot.add_node(pydot.Node(f'v{node_nums[v]}',
                                #label=v.label().name(),
                                label='',
                                shape='circle',
                                height=0.24,
    ...
    for e in g.edges():
        if e.label().is_terminal():
            dot.add_node(pydot.Node(f'e{edge_nums[e]}',
                                    label='',
                                    xlabel=e.label().name(),
                                    shape='square',
                                    height=0.16,
        ...
        for i, v in enumerate(e.nodes()):
            ...
            if format[0] == '>':
                dot.add_edge(pydot.Edge(f'e{edge_nums[e]}', f'v{node_nums[v]}',
                                        order=i+1,
                                        label=format[1],
                ))
            elif format[0] == '<':
                dot.add_edge(pydot.Edge(f'v{node_nums[v]}', f'e{edge_nums[e]}',
                                        order=i+1,
                                        label=format[1],
                ))
            elif format[0] == '^':
                sub = pydot.Subgraph(rank='same')
                sub.add_node(pydot.Node(f'v{node_nums[v]}'))
                sub.add_node(pydot.Node(f'e{edge_nums[e]}'))
    ...
    for i, v in enumerate(g.ext()):
        [dv] = dot.get_node(f'v{node_nums[v]}')
...
def factorgraph_to_tikz(g: FactorGraph, factor_formats=None, lhs=None):
    ...
    node_nums = {v:i for (i,v) in enumerate(g.nodes())}
    edge_nums = {e:i for (i,e) in enumerate(g.edges())}

    # Convert to DOT just to get layout information
    dot = factorgraph_to_dot(g, node_nums, edge_nums, factor_formats, lhs)
    ...
    ys = [positions[f'v{node_nums[v]}'][1] for v in g.nodes()]
    ys.extend([positions[f'e{node_nums[e]}'][1] for e in g.edges()])
    baseline = (min(ys)+max(ys))/2
    
    res = []
    res.append(rf'\begin{{tikzpicture}}[baseline={baseline}pt]')

    ext = {v:i for i,v in enumerate(g.ext())}
    for v in g.nodes():
        if v in ext:
            style = f'ext,label={_get_format(factor_formats, lhs, ext[v])[1]}'
        else:
            style = 'var'
        x, y = positions[f'v{node_nums[v]}']
        res.append(rf'  \node [{style}] (v{node_nums[v]}) at ({x}pt,{y}pt) {{}};')
    for e in g.edges():
        x, y = positions[f'e{edge_nums[e]}']
        if e.label().is_terminal():
            res.append(rf'  \node [fac,label={{{e.label().name()}}}] (e{edge_nums[e]}) at ({x}pt,{y}pt) {{}};')
        else:
            res.append(rf'  \node [fac] (e{edge_nums[e]}) at ({x}pt,{y}pt) {{{e.label().name()}}};')
        for i, v in enumerate(e.nodes()):
            label = _get_format(factor_formats, e.label(), i)[1]
            res.append(rf'    \draw (e{edge_nums[e]}) edge node[tent,near start] {{{label}}} (v{node_nums[v]});')
