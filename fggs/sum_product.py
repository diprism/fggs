...
class SumProduct():
    ...
    def __call__(self, *, method: str = 'fixed-point', tol: float = 1e-6, kmax: int = 1000) -> Tensor:
        ...
        if method == 'fixed-point':
            self._fixed_point(self._F, psi_X, tol=tol, kmax=kmax)
        elif method == 'newton':
            self._newton(lambda psi_X: self._F(psi_X) - psi_X, self._J, psi_X, tol=tol, kmax=kmax)
        elif method == 'broyden':
            ...
            self._broyden(lambda psi_X: self._F(psi_X) - psi_X, invJ, psi_X, tol=tol, kmax=kmax)
            # k = 1
